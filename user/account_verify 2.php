...
error_log("HDSI @account_verify.php #4: $this_page", 0);
?>
...
//                    TODO: validate email with email_db as double check
                        if (isset($result)) {
                            ...
                            echo "<scrip> showMessage('The email and verification token are received but something went wrong. Contact the Support Team for help if the problem persists. <br> Redirecting to HDSI Home in 10 seconds...')</scrip>";
...
// TODO: the confirm page style is from ??????
                    ...
                    <form method="post">
                        ...
                        <div style=" font-size: 1rem; background-color: #CEB888"> Please click the
                                                                            CONFIRM button below
                                                            to verify your HDSI account creation request
...
if (isset($_POST['submit_verify'])) {
    ...
    if ((isset($_POST['email_from_email'])) && (isset($_POST['token_from_email'])) && (isset($_POST['token_from_db']))) {
        ...
        if ($token_from_email == $token_from_db) {
            ...
            $stmt->execute();
            echo "
        <script> showMessage (' Congratulations! Your HDSI account creation is verified. <br> Redirecting your to HDSI Home in 5 seconds.'); </script>
        ";
            echo "<meta http-equiv=REFRESH CONTENT=5;url=$p>";
            exit;
        } else {
            echo "
        <script> showMessage (' The account verification is not successful (token incorrect). Please try again. Contact the HDSI Support Team if the issue persists. <br> Redirecting your to HDSI Home in 10 seconds.'); </script>
        ";
