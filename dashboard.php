...
<script defer src="https://hdsinet.us/scripts/taskMenu.js"></script>
<script defer src="https://hdsinet.us/scripts/memberMenu.js"></script>
<script defer src="https://hdsinet.us/scripts/fileMenu.js"></script>
<!-- ##### end of JS for task/member/file actions ##### -->

<?php
include_once("/var/www/hdsi/includes/headers.php");
include_once("/var/www/hdsi/includes/login_loader.php");
include_once("./includes/topnav.php");
include_once("./includes/utilities.php"); // #### facebook_time
include_once("/var/www/hdsi/scripts/modals.php"); // #### modals
            ...
            <div class="col-5">
                ...
                <div class="section-pane" style="width: 100%">
                    ...
                    <div class="pane-content">
                        ...
                        // INNER JOIN
                        // projects AS prj
                        // ON pu.id_project = prj.id_project WHERE pu.id_user = '$uid_hdsi' ORDER BY prj.date_to_complete
                        $result = $pdo->query(" 
...
WHERE id_user = '$uid_hbdi' ")->fetchAll();
                        foreach ($result as $row) {
                            ...
                            foreach ($result2 as $row2) {
                                ...
                                $username_hdsi = $pdo->query("SELECT username FROM user WHERE id_user = '$id_creator' ")->fetch();
                                $username_hdsi = $username_hdsi['username'];
                                ?>
                                <div class='content-item-wrap' style="font-weight: 600">
                                    ...
                                    <a style='text-decoration: none; color: #782F40'
                                       href='<?php echo $p; ?>/projects/<?php echo $username_hdsi; ?>/<?php echo $title_project_short; ?>.php'>
...
//                                    $id_project = $row['id_project'];
//                                    $projects = $pdo->query(" SELECT id_project FROM project_user WHERE id_user = $uid_hdsi")->fetchAll();
...
">
                    ...
                    <div class='pane-content'>
                        ...
                        // ##### get Projects where (uid_hdsi == id_user) DISTINCT from project_user
                        $projects = $pdo->query(" 
 SELECT DISTINCT id_project FROM project_user WHERE id_user = $uid_hbdi ")->fetchAll();
                        //                        $stmt->execute([$uid_hdsi]);
...
//         TODO: PDO parameterrized
    $sql = " INSERT INTO projects (id_creator, title_project, title_project_short, time_created, date_begin, date_to_complete, token_id_project_creation ) 
                    VALUES ($uid_hdsi, '$title_project', '$title_project_short', '$time_created', '$date_begin', '$date_to_complete', '$token_id_project_creation') ";
    $pdo->exec($sql);
...
//    $user_dir = $_SERVER['DOCUMENT_ROOT'] . "/hdsi/projects/$username_hdsi";
...
//    get id_project
    ...
    try {
        $stmt = " INSERT INTO project_user (id_user, id_project, role) VALUES ('$uid_hdsi', '$id_project', 'lead') ";
        $pdo->exec($stmt);
...
// ### create project_files directory
    $dir_project_files = $_SERVER['DOCUMENT_ROOT'] . "/hdsi/projects/$username_hdsi/$title_project_short" . '_files';
...
////                }
////                unlink($test_file);
////                $projects_dir = $_SERVER['DOCUMENT_ROOT'] . "hdsi/projects";
...
//  ### Project PHP viewer file to display project. "<br>";
    $project_index = $_SERVER['DOCUMENT_ROOT'] . "hdsi/projects/$username_hdsi/$title_project_short" . '.php';
...
// Project Creation a Success; redirecting.
    $msg = "HDSI project $title_project is successfully created. <br> Redirecting you to the Project Home in 5 seconds... ";
    echo "<script> showMessage(' $msg ') </script>";
    echo "<meta http-equiv=REFRESH content=5;url=$p/projects/$username_hdsi/$title_project_short.php>";
    exit();
}
?>
