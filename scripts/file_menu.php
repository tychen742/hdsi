...
// ##### uploadOK ==> insert DB #####
    if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($_FILES["userfile"]["tmp_name"], $target_file)) {
            ...
            $date_uploaded = time();
            $token_passing = strval($uid_hdsi) . '_' . strval(time());

            $sql = $pdo->prepare("
INSERT INTO files (id_project, uploaded_by, name_file, date_uploaded, token_passing)
VALUES('$id_project', '$uid_hdsi', '$file_name', '$date_uploaded', '$token_passing') ");
            if ($sql->execute()) {
                ...
                if ($compliance != null) {
                    foreach ($compliance as $compliance_data) {
                        ...
                        if (($compliance_data == 'dataset') || ($compliance_data == 'document')) {
                            ...
                            $sql = "INSERT INTO permission_store (id_project, id_user, id_file, permission) VALUES ('$id_project', '$uid_hdsi', '$id_file' , '$compliance_data') ";
                            $pdo->prepare($sql)->execute();
                        }
                    }
...
//            $stmt->execute();
            ...
            if (file_exists($target_file)) {
                ...
                $transaction = $pdo->prepare(" INSERT INTO transaction_store (id_user, ip_address, transaction_type, id_file) VALUES ('$uid_hdsi', '$ip_address', 'delete', '$id_file') ");
                $transaction->execute();
            } else {
